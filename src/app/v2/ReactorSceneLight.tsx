  ...
  useFrame((state) => {
    ...
    for (let ring = 0; ring < 6; ring++) {
      ...
      for (let p = 0; p < ringParticles; p++) {
        ...
        pColor.setHSL(hue, 0.3, 0.04 + brightness * 0.04);
    ...
    for (let p = 0; p < coreCount; p++) {
      ...
      pColor.setHSL(hue, 0.4, 0.05 + cBright * 0.05);
    ...
    for (let arc = 0; arc < 6; arc++) {
      ...
      for (let p = 0; p < arcCount; p++) {
        ...
        // Arcs: dark particles, fade at ends
        const fade = Math.sin(arcProgress * Math.PI);
        const flicker = 0.5 + Math.sin(time * 10 + p * 3) * 0.2;
        pColor.setHSL((hue + 0.05) % 1, 0.4, 0.03 + fade * flicker * 0.06);
...
export default function ReactorSceneLight({
  width,
  height,
}: {
  width: number;
