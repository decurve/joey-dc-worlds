...
function GearSystem() {
  ...
  const assignments = useMemo(() => {
    ...
    const teethCount = Math.floor(count * 0.2);
    const hubCount = Math.floor(count * 0.1);
    const ambientCount = count - ringCount - teethCount - hubCount;
  ...
  useFrame((state) => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (a.role === ROLE_AMBIENT) {
        ...
        // Ambient particles use rainbow hue at low saturation
        pColor.setHSL(hue, 0.3, 0.08 + shimmer * 0.04);
        target.set(
        ...
        if (a.role === ROLE_RING) {
          ...
          // Hub: moderate saturation, darkest visible
          pColor.setHSL(hue, 0.7, 0.55);
        }
      }
...
export default function GearParticleScene({
  width,
  height,
}: {
  width: number;
