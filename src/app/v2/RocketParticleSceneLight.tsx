...
const SCALE = 0.28;
const X_OFFSET = 0;
const CANVAS_SHIFT = "5%"; // shift entire canvas right
  ...
  useFrame((state) => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (u < 0.55) {
        ...
        lit = 0.02 + Math.pow(Math.max(0, Math.sin(theta + time * 1.5 - y * 0.05)), 8) * 0.06;
        sat = 0.3;
      } else if (u < 0.80) {
...
export default function RocketParticleSceneLight({
  width,
  height,
}: {
  width: number;
